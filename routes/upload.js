...
const createMask = require("../helpers/createMask.js");
			...
			s3.upload(params, async function (err, data) {
				...
				if (data) {
					...
					try {
						...
						const createMaskData = await createMask(solution._id, req.user._id);

						if (!createMaskData) {
							return res.status(500).json({ message: "Failed to create mask" });
						}

						solution.mask_job_id = createMaskData.jobId;

						await solution.save();

						responseSolution.mask_job_id = solution.mask_job_id;
